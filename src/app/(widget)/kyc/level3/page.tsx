...
export default function KycLevel3Page() {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/login?redirect=/kyc/level3')
      } else if (!user.customerId) {
        router.push('/customer/create?redirect=/kyc/level3')
      }
    }
  }, [user, loading, router])

  // Redirect if already completed Level 3 or don't have Level 2
  useEffect(() => {
    if (user?.kycData?.kycLevel === "Level 3") {
      router.push('/profile')
    } else if (user?.kycData?.kycLevel !== "Level 2" && user?.kycStatus === "COMPLETED") {
      router.push('/kyc/level2?redirect=/kyc/level3')
    }
  }, [user, router])
    ...
    return (
      <Card className="border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,0.8)] bg-white p-6">
        <div className="flex flex-col items-center gap-6">
          ...
          <Button 
            onClick={() => router.push('/login?redirect=/kyc/level3')}
  ...
  // If Level 2 is not completed, show a prompt to complete it first
  if (user.kycData?.kycLevel !== "Level 2" && user.kycStatus === "COMPLETED") {
    return (
      <Card className="border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,0.8)] bg-white p-6">
        <div className="flex flex-col items-center gap-6">
          ...
          <h1 className="text-2xl font-bold text-center">Complete Level 2 First</h1>
          <p className="text-center text-gray-600">
            You need to complete KYC Level 2 verification before proceeding to Level 3.
          </p>
          <Button 
            onClick={() => router.push('/kyc/level2?redirect=/kyc/level3')}
            className="w-full bg-black text-white font-bold border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,0.8)] hover:translate-y-1 hover:translate-x-1 hover:shadow-none transition-all"
          >
